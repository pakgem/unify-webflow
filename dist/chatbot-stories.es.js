...
const _l = [
...
], Al = 1.28, S = (r) => Number((r * Al).toFixed(3)), vl = [3, 3, 3], Sl = {
  ...
  contentWidth: 1881,
  height: 1280
}, Nl = 96, _n = ".wa-cursor-file, .wa-file-landing-clone, .wa-file-landing-label, .wa-csv-drop, .wa-signup-logo-transfer", An = "[data-marketing-data-sources-grid]", vn = "[data-data-table]", Lt = "[data-table-action]", Ka = "[data-table-page-button]", Xa = "[data-table-page-range]", ke = {
  duration: S(0.12),
  incomingLag: S(0.1),
  rowStagger: S(0.03),
...
}, Ol = "https://cdn.jsdelivr.net/npm/simple-icons@latest/icons", Bl = "https://www.google.com/s2/favicons", zl = {
  adyen: "adyen",
  airtable: "airtable",
  brex: "brex",
  "dbt labs": "dbt",
...
class Ul {
  ...
  strategyPlans(e) {
    ...
    const d = this.claimComponentMessage("strategy", a), u = m.timeline().add(this.revealMessage(d, n, yn)).addLabel(s, Ll).call(() => this.setMotionHints(c), void 0, s).to(t, { ...o.to }, s);
  ...
  createMarketingDataSourcesGrid(e) {
    ...
    return t.classList.add("wa-data-source-grid--marketing"), t.dataset.marketingDataSourcesGrid = e.id, a.className = "wa-data-source-grid__scale", i.className = "wa-data-source-grid__groups", ql.forEach((s) => {
      const l = document.createElement("div");
  ...
  createSequenceEngagement(e) {
    ...
    e.sequences.forEach((p, f) => {
      ...
      const v = p.steps;
      if (v?.length) {
        const C = document.createElement("div"), k = this.getInitialSequenceStepIndex(e, v.length), M = document.createElement("div"), F = document.createElement("span"), z = document.createElement("strong"), L = document.createElement("div");
...
function sa(r, e, t) {
  ...
  const s = e.x - r.x, l = e.y - r.y, c = s / i, d = l / i, u = -d, g = c, p = a() > 0.5 ? 1 : -1, f = t.curve ?? 1, w = zt(i * (o === "drag" ? 0.1 : o === "click" ? 0.17 : 0.22) * f, 18, 150) * p * (0.72 + a() * 0.44), b = i / Wl[n] + 0.16, y = zt(
    b * Yl[o] * Ql * (t.durationScale ?? 1),
    0.3,
    1.98
...
}, rc = 2.8, oc = 42, sc = 2, lc = 3, Ca = "[data-chat-shell] [data-chat-thread]", cc = `${Ca} [data-message-role="assistant"]:not(.wa-message--component) [data-message-body]`, dc = `${Ca} [data-message-role="assistant"].wa-message--thinking .wa-research-step[data-step-state="current"] .wa-research-step__body, ${Ca} [data-message-role="assistant"].wa-message--thinking .wa-research-step[data-step-state="complete"] .wa-research-step__label`, xi = 0.24, ao = 0.3, Fe = {
...
function Rc(r, e) {
  ...
  ), o = a ? e.steps.indexOf(a) : -1, s = n.find((f) => o >= 0 && e.steps.indexOf(f) < o), l = [...n].reverse().find((f) => f !== s), c = t?.text || "may_webinar_attendees.csv", d = Dc(t, s?.component ?? l?.component), u = r.chat.prepareCsvDropArea(), g = r.chat.prepareCursorFile(c, r.cursor), p = B("[data-chat-shell]", "center", {
    desktop: { x: 0, y: 82 },
    tablet: { x: 0, y: 72 },
    mobile: { x: 0, y: 64 }
  });
  return Re(r, [
    ...
    ...l ? [{ kind: "dataTable", config: Uc(ea(l.component, "cleaned-webinar-attendees")), at: "-=0.04" }] : [],
...
function Bc(r, e, t) {
  ...
  if (a.kind === "table") {
    const i = ea(a, `${e}-${t.id}`);
    e === "data-marketplace" && Gc(i), Qc(e, t, i) && (i.scrollAlign = "equal-inset"), r.push({ kind: "dataTable", config: i, at: "-=0.04" });
    return;
  }
...
function Ia(r, e = {}) {
  return {
    kind: "thinking",
    thinking: zc(r.thinking, r.text, r.note),
    hold: e.hold,
    at: e.at
  };
}
...
function In(r) {
  r.name && "company" in r && (r.company = "Stripe"), r.prospectDetail && (r.prospectDetail = Vc(r.prospectDetail)), r.connector && (r.connector = Hc(r.connector));
}
function Vc(r) {
  const e = r.trim();
...
function jc(r, e) {
  const t = r.columns, a = t.findIndex((d) => d.trim().toLowerCase() === "name"), i = a >= 0 ? t.findIndex((d, u) => u > a && /^role\b/i.test(d.trim())) : -1, n = a >= 0 && i >= 0, o = co(e) ? t.findIndex((d) => d.trim().toLowerCase() === "title") : -1, s = n ? i : o >= 0 ? o : void 0, l = t.map((d, u) => u).filter((d) => d !== i && d !== o), c = l.map((d) => {
    ...
    const g = Xc(e, u), p = g.key ?? ie(u || `column-${d + 1}`);
    return {
      key: p,
      label: u,
...
function uo(r) {
  const [e = "", t = ""] = r.split(/\s+[—–]\s+/, 2), a = e.trim().match(/^(.+?)(?:\s*\((.+)\))?$/), i = a?.[1]?.trim() || r.trim(), n = t.trim(), o = Fc[ie(i).replace(/-/g, " ")], s = td(a?.[2]?.trim() || "", o, n);
  return {
    name: i,
    title: s.title,
    context: n,
...
class bd {
  ...
  updateFollowTarget(e) {
    ...
    if (this.lastPointer) {
      ...
      i > 1.5 && (this.trailDirection = {
        x: a.x / i,
        y: a.y / i
      }, this.velocity = Sd(a, $.maxMomentumStep));
    }
    const t = {
  ...
  updatePlayTarget(e) {
    ...
    this.target = _d(n, this.pointer, He.minPointerDistance, He.viewportInset);
  }
  resumeFollowing(e) {
  ...
  isCursorTooFarFromBrowser() {
    ...
    return vd(i, e) > $.maxBrowserDistance;
  }
  getChatShell() {
    return this.chatShell?.isConnected ? this.chatShell : (this.chatShell = this.root.querySelector("[data-chat-shell]"), this.chatShell);
  }
...
function Id(r) {
  ...
  const e = Ae(r.id), t = Ae(r.label), a = Ae(r.summary), i = r.steps.map((n) => Ld(n)).filter((n) => !!n);
...
function Ld(r) {
  ...
  return !e || !t || !Rd(t) ? null : {
    ...
    text: a ?? "",
    note: i ?? "",
    thinking: kt(r.thinking) ? qd(r.thinking, a ?? "", i ?? "") : t === "thinking" ? Ao(a ?? "", i ?? "") : void 0,
...
const Fd = "[data-chatbot-stories]", Ud = "[data-chatbot-stories][data-auto-init]";
function Gd(r, {

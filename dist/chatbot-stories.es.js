...
var me = {
  ...
  return X(i) ? d = u = {
    ...
    var h = (f || t).length, w = r[h], b, y, _, v, x, T, A, C, k;
    if (!w) {
      if (k = t.grid === "auto" ? 0 : (t.grid || [1, _e])[1], !k) {
        for (A = -_e; A < (A = f[k++].getBoundingClientRect().left) && k < h; )
          ;
        k < h && k--;
      }
      for (w = r[h] = [], b = l ? Math.min(k, h) * d - 0.5 : i % k, y = k === _e ? 0 : l ? h * u / k - 0.5 : i / k | 0, A = 0, C = _e, T = 0; T < h; T++)
        _ = T % k - b, v = y - (T / k | 0), w[T] = x = c ? Math.abs(c === "y" ? v : _) : Xn(_ * _ + v * v), x > A && (A = x), x < C && (C = x);
      i === "random" && pr(w), w.max = A - C, w.min = C, w.v = h = (parseFloat(t.amount) || parseFloat(t.each) * (k > h ? h - 1 : c ? c === "y" ? h / k : k : Math.max(k, h / k)) || 0) * (i === "edges" ? -1 : 1), w.b = h < 0 ? n - h : n, w.u = te(t.amount || t.each) || 0, a = a && h < 0 ? as(a) : a;
    }
  ...
  return t.to = function(i, n, r) {
    ...
    if (this !== G && d > l && i >= 0 && (d = l), d !== this._tTime || r || u) {
      if (s !== this._time && c && (d += this._time - s, i += this._time - s), g = d, v = this._start, _ = this._ts, b = !_, u && (c || (s = this._zTime), (i || !n) && (this._zTime = i)), this._repeat) {
        ...
        if (g = V(d % w), d === l ? (h = this._repeat, g = c) : (x = V(d / w), h = ~~x, h && h === x && (g = c, h--), g > c && (g = c)), x = Ct(this._tTime, w), !s && this._tTime && x !== h && this._tTime - x * w - this._dur <= 0 && (x = h), T && h & 1 && (g = c - g, A = 1), h !== x && !this._lock) {
          var C = T && x & 1, k = C === (T && h & 1);
          if (h < x && (C = !C), s = C ? 0 : d % c ? c : d, this._lock = 1, this.render(s || (A ? 0 : V(h * w)), n, !c)._lock = 0, this._tTime = d, !n && this.parent && pe(this, "onRepeat"), this.vars.repeatRefresh && !A && (this.invalidate()._lock = 1, x = h), s && s !== this._time || b !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (c = this._dur, l = this._tDur, k && (this._lock = 2, s = C ? c : -1e-4, this.render(s, !0), this.vars.repeatRefresh && !A && this.invalidate()), this._lock = 0, !this._ts && !b)
            return this;
        }
      }
...
var is = function(e, t, a, i, n, r, s) {
  ...
  var i = e.vars, n = i.ease, r = i.startAt, s = i.immediateRender, l = i.lazy, c = i.onUpdate, d = i.runBackwards, u = i.yoyoEase, g = i.keyframes, p = i.autoRevert, f = e._dur, h = e._startAt, w = e._targets, b = e.parent, y = b && b.data === "nested" ? b.vars.targets : w, _ = e._overwrite === "auto" && !vi, v = e.timeline, x = i.easeReverse || u, T, A, C, k, M, z, F, q, D, L, P, U, $;
  if (v && (!g || !n) && (n = "none"), e._ease = gt(n, Gt.ease), e._rEase = x && (gt(x) || e._ease), e._from = !v && !!i.runBackwards, e._from && (e.ratio = 1), !v || g && !i.stagger) {
    if (q = w[0] ? ht(w[0]).harness : 0, U = q && i[q.prop], T = xa(i, Ti), h && (h._zTime < 0 && h.progress(1), t < 0 && d && s && !p ? h.render(-1, !0) : h.revert(d && f ? pa : Ro), h._lazy = 0), r) {
      ...
      }, T), U && (C[q.prop] = U), $e(e._startAt = K.set(w, C)), e._startAt._dp = 0, e._startAt._sat = e, t < 0 && (Z ? e._startAt.revert(pa) : e._startAt.render(-1, !0)), e._zTime = t, !s)
        o(e._startAt, O, O);
      else if (!t)
        return;
    }
    for (e._pt = e._ptCache = 0, l = f && oe(l) || l && !f, A = 0; A < w.length; A++) {
      if (M = w[A], F = M._gsap || Mi(w)[A]._gsap, e._ptLookup[A] = L = {}, li[F.id] && Je.length && ya(), P = y === w ? A : y.indexOf(M), q && (D = new q()).init(M, U || T, e, P, y) !== !1 && (e._pt = k = new le(e._pt, M, D.name, 0, 1, D.render, D, 0, D.priority), D._props.forEach(function(qe) {
        L[qe] = k;
      }), D.priority && (z = 1)), !q || U)
        for (C in T)
...
var K = /* @__PURE__ */ (function(o) {
  ...
  function e(a, i, n, r) {
    ...
    var l = s.vars, c = l.duration, d = l.delay, u = l.immediateRender, g = l.stagger, p = l.overwrite, f = l.keyframes, h = l.defaults, w = l.scrollTrigger, b = i.parent || G, y = (ae(a) || Jn(a) ? He(a[0]) : "length" in i) ? [a] : ve(a), _, v, x, T, A, C, k, M;
    if (s._targets = y.length ? Mi(y) : Wt("GSAP target " + a + " not found. https://gsap.com", !me.nullTargetWarn) || [], s._ptLookup = [], s._overwrite = p, f || g || na(c) || na(d)) {
      ...
      if (_ = s.timeline = new re({
        ...
        for (v = 0; v < T; v++)
          x = xa(i, Er), x.stagger = 0, z && (x.easeReverse = z), M && kt(x, M), C = y[v], x.duration = +Ht(c, Oe(s), v, C, y), x.delay = (+Ht(d, Oe(s), v, C, y) || 0) - s._delay, !g && T === 1 && x.delay && (s._delay = d = x.delay, s._start += d, x.delay = 0), _.to(C, x, k ? k(v, C, y) : 0), _._ease = R.none;
...
var Zi, je, _t, Ni, ut, $i, Oi, ys = function() {
  ...
  */
  ...
  if (_ && (u !== rt || d !== rt)) {
    var T = parseFloat(d) * vt, A = Math.sin(T), C = Math.cos(T), k;
    T = parseFloat(u) * vt, k = Math.cos(T), r = Ua(y, r, A * k * -_), s = Ua(y, s, -Math.sin(T) * -_), l = Ua(y, l, C * k * -_ + _);
  }
  ...
  var a = t || this, i = a.xPercent, n = a.yPercent, r = a.x, s = a.y, l = a.rotation, c = a.skewX, d = a.skewY, u = a.scaleX, g = a.scaleY, p = a.target, f = a.xOrigin, h = a.yOrigin, w = a.xOffset, b = a.yOffset, y = a.forceCSS, _ = parseFloat(r), v = parseFloat(s), x, T, A, C, k;
  l = parseFloat(l), c = parseFloat(c), d = parseFloat(d), d && (d = parseFloat(d), c += d, l += d), l || c ? (l *= vt, c *= vt, x = Math.cos(l) * u, T = Math.sin(l) * u, A = Math.sin(l - c) * -g, C = Math.cos(l - c) * g, c && (d *= vt, k = Math.tan(c - d), k = Math.sqrt(1 + k * k), A *= k, C *= k, d && (k = Math.tan(d), k = Math.sqrt(1 + k * k), x *= k, T *= k)), x = Q(x), T = Q(T), A = Q(A), C = Q(C)) : (x = u, C = g, T = A = 0), (_ && !~(r + "").indexOf("px") || v && !~(s + "").indexOf("px")) && (_ = tt(p, "x", r, "px"), v = tt(p, "y", s, "px")), (f || h || w || b) && (_ = Q(_ + f - (f * x + h * A) + w), v = Q(v + h - (f * T + h * C) + b)), (i || n) && (k = p.getBBox(), _ = Q(_ + i / 100 * k.width), v = Q(v + n / 100 * k.height)), k = "matrix(" + x + "," + T + "," + A + "," + C + "," + _ + "," + v + ")", p.setAttribute("transform", k), y && (p.style[W] = k);
...
var Gr = {
  ...
  init: function(e, t, a, i, n) {
    var r = this._props, s = e.style, l = a.vars.startAt, c, d, u, g, p, f, h, w, b, y, _, v, x, T, A, C, k;
      ...
      if (h !== "autoRound" && (d = t[h], !(ue[h] && Cr(h, t, a, i, e, n)))) {
        if (p = typeof d, f = Aa[h], p === "function" && (d = d.call(a, i, e, n), p = typeof d), p === "string" && ~d.indexOf("random(") && (d = Qt(d)), f)
          ...
          if (_ || (u || u === 0) && (g || g === 0) && !_s.test(d) && h in s)
            w = (c + "").substr((g + "").length), u || (u = 0), b = te(d) || (h in me.units ? me.units[h] : w), w !== b && (g = tt(e, h, c, b)), this._pt = new le(this._pt, _ ? x : s, h, g, (y ? xt(g, y + u) : u) - g, !_ && (b === "px" || h === "zIndex") && t.autoRound !== !1 ? ks : fi), this._pt.u = b || 0, _ && k !== d ? (this._pt.b = c, this._pt.e = k, this._pt.r = Ss) : w !== b && b !== "%" && (this._pt.b = c, this._pt.r = As);
          else if (h in s)
...
class Ul {
  ...
  createSequenceEngagement(e) {
    ...
    e.sequences.forEach((p, f) => {
      ...
      const A = p.steps;
      if (A?.length) {
        const C = document.createElement("div"), k = this.getInitialSequenceStepIndex(e, A.length), M = document.createElement("div"), z = document.createElement("span"), F = document.createElement("strong"), q = document.createElement("div");
        C.className = "wa-sequence-steps", A.forEach((D, L) => {

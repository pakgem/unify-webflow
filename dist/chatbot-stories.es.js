...
var pe = {
  ...
  return u = {
    ...
    wake: function() {
      kr && (!Xa && hi() && (Ce = Xa = window, mi = Ce.document || {}, he.gsap = oe, (Ce.gsapVersions || (Ce.gsapVersions = [])).push(oe.version), Sr(ha || Ce.GreenSockGlobals || !Ce.gsap && Ce || {}), Gr.forEach(Ur)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, l && u.sleep(), d = c || function(f) {
    ...
    add: function(f, b, y) {
      ...
      return u.remove(f), s[y ? "unshift" : "push"](_), It(), _;
    },
    remove: function(f, b) {
  ...
  return t.to = function(i, r, o) {
    ...
    if (this !== V && c > l && i >= 0 && (c = l), c !== this._tTime || o || u) {
      if (s !== this._time && d && (c += this._time - s, i += this._time - s), p = c, v = this._start, _ = this._ts, b = !_, u && (d || (s = this._zTime), (i || !r) && (this._zTime = i)), this._repeat) {
        ...
        if (p = U(c % f), c === l ? (h = this._repeat, p = d) : (x = U(c / f), h = ~~x, h && h === x && (p = d, h--), p > d && (p = d)), x = Pt(this._tTime, f), !s && this._tTime && x !== h && this._tTime - x * f - this._dur <= 0 && (x = h), T && h & 1 && (p = d - p, k = 1), h !== x && !this._lock) {
...
var yo = function(e, t, a, i, r, o, s) {
  ...
  if (v && (!p || !r) && (r = "none"), e._ease = pt(r, jt.ease), e._rEase = x && (pt(x) || e._ease), e._from = !v && !!i.runBackwards, e._from && (e.ratio = 1), !v || p && !i.stagger) {
    if (I = f[0] ? ct(f[0]).harness : 0, z = I && i[I.prop], T = ga(i, fi), h && (h._zTime < 0 && h.progress(1), t < 0 && c && s && !m ? h.render(-1, !0) : h.revert(c && g ? ca : jn), h._lazy = 0), o) {
      ...
      }, T), z && (C[I.prop] = z), Xe(e._startAt = $.set(f, C)), e._startAt._dp = 0, e._startAt._sat = e, t < 0 && (K ? e._startAt.revert(ca) : e._startAt.render(-1, !0)), e._zTime = t, !s)
        n(e._startAt, O, O);
      else if (!t)
        return;
    }
    for (e._pt = e._ptCache = 0, l = g && ae(l) || l && !g, k = 0; k < f.length; k++) {
      if (E = f[k], L = E._gsap || bi(f)[k]._gsap, e._ptLookup[k] = M = {}, Ka[L.id] && je.length && ma(), G = y === f ? k : y.indexOf(E), I && (N = new I()).init(E, z || T, e, G, y) !== !1 && (e._pt = S = new re(e._pt, E, N.name, 0, 1, N.render, N, 0, N.priority), N._props.forEach(function(fe) {
        M[fe] = S;
      }), N.priority && (B = 1)), !I || z)
        for (C in T)
...
var $ = /* @__PURE__ */ (function(n) {
  ...
  function e(a, i, r, o) {
    ...
    var l = s.vars, d = l.duration, c = l.delay, u = l.immediateRender, p = l.stagger, m = l.overwrite, g = l.keyframes, h = l.defaults, f = l.scrollTrigger, b = i.parent || V, y = (Z(a) || _r(a) ? Oe(a[0]) : "length" in i) ? [a] : xe(a), _, v, x, T, k, C, S, E;
    if (s._targets = y.length ? bi(y) : Yt("GSAP target " + a + " not found. https://gsap.com", !pe.nullTargetWarn) || [], s._ptLookup = [], s._overwrite = m, g || p || aa(d) || aa(c)) {
      ...
      s.timeline = 0;
    return m === !0 && !ui && (Ge = Ne(s), V.killTweensOf(y), Ge = 0), Te(b, Ne(s), r), i.reversed && s.reverse(), i.paused && s.paused(!0), (u || !d && !g && s._start === U(b._time) && ae(u) && Qn(Ne(s)) && b.data !== "nested") && (s._tTime = -O, s.render(Math.max(0, -c) || 0)), f && Nr(Ne(s), f), s;
  }
  var t = e.prototype;
  return t.render = function(i, r, o) {
    ...
    if (!d)
      ...
      if (p = u, _ = this.timeline, this._repeat) {
        ...
        if (p = U(u % h), u === l ? (g = this._repeat, p = d) : (f = U(u / h), g = ~~f, g && g === f ? (p = d, g--) : p > d && (p = d)), b = this._yoyo && g & 1, b && (p = d - p), f = Pt(this._tTime, h), p === s && !o && this._initted && g === f)
...
var Gi, Ue, Ct, Ci, dt, Ui, Ai, Lo = function() {
  ...
  if (s.b = a, s.e = i, a += "", i += "", i.substring(0, 6) === "var(--" && (i = ue(e, i.substring(4, i.indexOf(")")))), i === "auto" && (h = e.style[t], e.style[t] = i, i = ue(e, t) || i, h ? e.style[t] = h : Ke(e, t)), c = [a, i], jr(c), a = c[0], i = c[1], p = a.match(vt) || [], x = i.match(vt) || [], x.length) {
    for (; u = vt.exec(i); )
      f = u[0], y = i.substring(l, u.index), g ? g = (g + 1) % 5 : (y.substr(-5) === "rgba(" || y.substr(-5) === "hsla(") && (g = 1), f !== (h = p[d++] || "") && (m = parseFloat(h) || 0, v = h.substr((m + "").length), f.charAt(1) === "=" && (f = St(m, f) + v), b = parseFloat(f), _ = f.substr((b + "").length), l = vt.lastIndex - _.length, _ || (_ = _ || pe.units[t] || v, l === i.length && (i += _, s.e += _)), v !== _ && (m = Je(e, t, h, _) || 0), s._pt = {
        _next: s._pt,
        p: y || d === 1 ? y : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: m,
  ...
  */
  ...
  f !== Bt && (v += "perspective(" + f + at), (i || r) && (v += "translate(" + i + "%, " + r + "%) "), (x || o !== Bt || s !== Bt || l !== Bt) && (v += l !== Bt || x ? "translate3d(" + o + ", " + s + ", " + l + ") " : "translate(" + o + ", " + s + at), d !== tt && (v += "rotate(" + d + at), c !== tt && (v += "rotateY(" + c + at), u !== tt && (v += "rotateX(" + u + at), (p !== tt || m !== tt) && (v += "skew(" + p + ", " + m + at), (g !== 1 || h !== 1) && (v += "scale(" + g + ", " + h + at), y.style[W] = v || "translate(0, 0)";
}, ts = function(e, t) {
  ...
  l = parseFloat(l), d = parseFloat(d), c = parseFloat(c), c && (c = parseFloat(c), d += c, l += c), l || d ? (l *= At, d *= At, x = Math.cos(l) * u, T = Math.sin(l) * u, k = Math.sin(l - d) * -p, C = Math.cos(l - d) * p, d && (c *= At, S = Math.tan(d - c), S = Math.sqrt(1 + S * S), k *= S, C *= S, c && (S = Math.tan(c), S = Math.sqrt(1 + S * S), x *= S, T *= S)), x = Y(x), T = Y(T), k = Y(k), C = Y(C)) : (x = u, C = p, T = k = 0), (_ && !~(o + "").indexOf("px") || v && !~(s + "").indexOf("px")) && (_ = Je(m, "x", o, "px"), v = Je(m, "y", s, "px")), (g || h || f || b) && (_ = Y(_ + g - (g * x + h * k) + f), v = Y(v + h - (g * T + h * C) + b)), (i || r) && (S = m.getBBox(), _ = Y(_ + i / 100 * S.width), v = Y(v + r / 100 * S.height)), S = "matrix(" + x + "," + T + "," + k + "," + C + "," + _ + "," + v + ")", m.setAttribute("transform", S), y && (m.style[W] = S);
    ...
    for (h in t)
      if (h !== "autoRound" && (c = t[h], !(le[h] && Xr(h, t, a, i, e, r)))) {
        if (m = typeof c, g = ba[h], m === "function" && (c = c.call(a, i, e, r), m = typeof c), m === "string" && ~c.indexOf("random(") && (c = Xt(c)), g)
          ...
          d = (getComputedStyle(e).getPropertyValue(h) + "").trim(), c += "", Ye.lastIndex = 0, Ye.test(d) || (f = Q(d), b = Q(c), b ? f !== b && (d = Je(e, h, d, b) + b) : f && (c += f)), this.add(s, "setProperty", d, c, i, r, 0, 0, h), o.push(h), C.push(h, 0, s[h]);
        else if (m !== "undefined") {
          ...
          if (_ || (u || u === 0) && (p || p === 0) && !Oo.test(c) && h in s)
            f = (d + "").substr((p + "").length), u || (u = 0), b = Q(c) || (h in pe.units ? pe.units[h] : f), f !== b && (p = Je(e, h, d, b)), this._pt = new re(this._pt, _ ? x : s, h, p, (y ? St(p, y + u) : u) - p, !_ && (b === "px" || h === "zIndex") && t.autoRound !== !1 ? Ho : ri), this._pt.u = b || 0, _ && S !== c ? (this._pt.b = d, this._pt.e = S, this._pt.r = Fo) : f !== b && b !== "%" && (this._pt.b = d, this._pt.r = zo);
          else if (h in s)
...
class As {
  ...
  prepareThinkingMessage(e, t, a) {
    ...
    }), w.set(e.header, { autoAlpha: 0, y: 5 }), w.set(e.traveler, { autoAlpha: 0, x: 0, y: 0 }), w.set(e.steps, { display: "grid", autoAlpha: 1, y: 0 }), this.resetThinkingGuide(e), w.set(t, { autoAlpha: 0, y: a, display: "none" });
  }
...
function ra(n, e, t) {
  ...
  const s = e.x - n.x, l = e.y - n.y, d = s / i, c = l / i, u = -c, p = d, m = a() > 0.5 ? 1 : -1, g = t.curve ?? 1, f = Gt(i * (o === "drag" ? 0.1 : o === "click" ? 0.17 : 0.22) * g, 18, 150) * m * (0.72 + a() * 0.44), b = i / Is[r] + 0.16, y = Gt(b * Rs[o] * Ns, 0.3, 1.66), _ = t.overshoot === !1 || i < 120 ? 0 : typeof t.overshoot == "number" ? t.overshoot : Gt(i * 0.026, 4, 18), v = _ > 0 ? {
    x: e.x + d * _,
    y: e.y + c * _
  } : e, x = {
